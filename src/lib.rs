...
pub fn latlon_to_amateur_radio_great_circle_map(latitude_delta: f64, longitude_delta: f64) -> (f64, f64) {
    ...
    let distance: f64 = (square(latitude_delta_radian_sine) + square(longitude_delta_radian_sine)).sqrt();
    let k: f64 = match distance {
        0_f64 => 0_f64,
        _ => (if hemispheres_anterior_or_posterior {radian_to_degree(distance.asin())} else {180_f64 - radian_to_degree(distance.asin())}) / distance,
    };
    
    #[cfg(test)]
    dbg!(distance, k);
    
    (longitude_delta_radian_sine * k, latitude_delta_radian_sine * k)
}
